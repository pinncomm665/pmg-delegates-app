...
// POST { linkedin_url } → fast duplicate probe before submitting the form.
// Proxies to pmg-agent /api/internal/intake-check.
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${AGENT_BASE}/api/internal/intake-check`, {
      method: "POST",
