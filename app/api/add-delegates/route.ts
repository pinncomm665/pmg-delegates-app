...
// POST { event_id, contact_ids } → creates Delegate role rows (stage=identified)
// via pmg-agent assign_role. Human-gated (explicit select + confirm). Does NOT invite.
export async function POST(request: NextRequest) {
  ...
  try {
    const res = await fetch(`${AGENT_BASE}/api/internal/add-delegates`, {
      method: "POST",
